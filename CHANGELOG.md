...
- **The legacy standalone loaders in `hunyuan_quantized_nodes.py` (`HunyuanImage3QuantizedLoader`, `HunyuanImage3Int8Loader`, `HunyuanImage3NF4LoaderLowVRAMBudget`, `HunyuanImage3Int8LoaderBudget`) never called `repair_unquantized_bnb_modules()` — crashed with `AssertionError` deep in bitsandbytes' `fix_4bit_weight_quant_state_from_module` on the first real forward pass**: found via an actual NF4-v2 generation on real GPU hardware. Issues #36/#41's fix (current transformers' `should_convert_module()` only matches skip-list entries by prefix/suffix, not substring, so `shared_mlp`/`mlp.gate` — interior path segments in this checkpoint — silently never match and get wrongly wrapped as `Linear4bit`/`Linear8bitLt` despite being saved full-precision on disk) was only ever wired into `CleanModelLoader`/`hunyuan_loader_clean.py` and the Instruct loader; these 4 older, still-actively-used nodes were missed entirely. Added the same `repair_unquantized_bnb_modules()` call (NF4 paths: immediately followed by `apply_nf4_transformers_compat()`, same ordering requirement as the other loaders — repair must run first, or the compat shim's `module.cuda()` call would have bitsandbytes silently *real-quantize* `shared_mlp` instead of restoring it to full precision) to all 5 `from_pretrained()` call sites across the 4 classes. New parametrized static tests in `tests/test_regressions_static.py` assert every one of these classes calls `repair_unquantized_bnb_modules()` (and, for the NF4 ones, in the correct order relative to `apply_nf4_transformers_compat()`) so this can't silently regress again.
